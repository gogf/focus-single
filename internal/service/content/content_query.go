...
// Search 搜索内容列表
func Search(ctx context.Context, in SearchInput) (out *SearchOutput, err error) {
	var (
...
// GetDetail 查询详情
func GetDetail(ctx context.Context, id uint) (out *GetDetailOutput, err error) {
	out = &GetDetailOutput{}
...
// Create 创建内容
func Create(ctx context.Context, in CreateInput) (out CreateOutput, err error) {
	if in.UserId == 0 {
	...
	return CreateOutput{ContentId: uint(lastInsertID)}, err
}

// Update 修改
func Update(ctx context.Context, in UpdateInput) error {
	// 不允许HTML代码
	if err := ghtml.SpecialCharsMapOrStruct(in); err != nil {
		return err
	}
