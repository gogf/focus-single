...
// 前台系统-获取面包屑列表
func GetBreadCrumb(ctx context.Context, in *GetBreadCrumbInput) []BreadCrumb {
	...
	if in.ContentType != "" {
		...
		if topMenuItem != nil {
			breadcrumb = append(breadcrumb, BreadCrumb{
...
// 前台系统-获取标题
func GetTitle(ctx context.Context, in *GetTitleInput) string {
	var (
		titleArray []string
		uriPrefix  string
	)
...
// 渲染指定模板页面
func RenderTpl(ctx context.Context, tpl string, data ...View) {
	var (
		viewObj  = View{}
		viewData = make(g.Map)
...
// 渲染默认模板页面
func Render(ctx context.Context, data ...View) {
	RenderTpl(ctx, g.Cfg().MustGet(ctx, "viewer.indexLayout").String(), data...)
}

// 跳转中间页面
func Render302(ctx context.Context, data ...View) {
	view := View{}
	if len(data) > 0 {
		view = data[0]
	}
...
// 401页面
func Render401(ctx context.Context, data ...View) {
	view := View{}
	if len(data) > 0 {
		view = data[0]
	}
...
// 403页面
func Render403(ctx context.Context, data ...View) {
	view := View{}
	if len(data) > 0 {
		view = data[0]
	}
...
// 404页面
func Render404(ctx context.Context, data ...View) {
	view := View{}
	if len(data) > 0 {
		view = data[0]
	}
...
// 500页面
func Render500(ctx context.Context, data ...View) {
	view := View{}
	if len(data) > 0 {
		view = data[0]
	}
