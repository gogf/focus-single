...
func Update(ctx context.Context, in model.ContentUpdateInput) error {
	return dao.Content.Transaction(ctx, func(ctx context.Context, tx *gdb.TX) error {
		...
		_, err := dao.Content.
			...
			Where(dao.Content.Columns().UserId, bizctx.Get(ctx).User.Id).
			Update()
		return err
	})
}
...
func Delete(ctx context.Context, id uint) error {
	return dao.Content.Transaction(ctx, func(ctx context.Context, tx *gdb.TX) error {
		...
		_, err := dao.Content.Ctx(ctx).Where(g.Map{
			...
			dao.Content.Columns().UserId: bizctx.Get(ctx).User.Id,
		}).Delete()
		// 删除评论
		if err == nil {
...
// AdoptReply 采纳回复
func AdoptReply(ctx context.Context, id uint, replyID uint) error {
	return dao.Content.Transaction(ctx, func(ctx context.Context, tx *gdb.TX) error {
